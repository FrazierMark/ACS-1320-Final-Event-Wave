...
export const createGoogleEvent = (responseData) => {
    ...
    const extended_address = responseData.events[0].venue.extended_address.replace(/ /g, "+");
    const googleEventTitle = title.replace(/ /g, "+");
    const googleEventStart = pub.replace(/[^\w\s]|_/g, "").replace(/\s+/g, "");
    const googleEventEnd = String(parseInt(googleEventStart.replace("T", "")) + 1);
    const googleEventEndFormatted = googleEventEnd.slice(0, 8) + 'T' + googleEventEnd.slice(8);
